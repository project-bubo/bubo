...
//                die();
    ...
    public function getParamValues($treeNodeId, $identifier) {
        ...
        foreach ($p as $extTreeNodeId => $langData) {
            foreach ($langData as $lang => $data) {
                if (\Bubo\Utils\MultiValues::unserialize($data['param_value']) !== FALSE) {
                    $out['ext_tree_node_id_'.$extTreeNodeId][$lang] = \Bubo\Utils\MultiValues::unserialize($data['param_value']);
                } else {
        ...
        foreach ($p as $extTreeNodeId => $_p) {
            if ($_p['parent'] !== NULL) {
                $ret['children'][$_p['parent']][$extTreeNodeId] = array(
                            ...
                            'parent' => $_p['parent'],
                            'name'  => $_p['param_name'],
                            'value' => \Bubo\Utils\MultiValues::unserialize($_p['param_value']) ? \Bubo\Utils\MultiValues::unserialize($_p['param_value']) : $_p['param_value']
                );
            } else {
    ...
    public function getNamedParamValues($parentTreeNodeId, $treeNodeId, $identifier, $lang, $tag) {
        ...
        $s = \Bubo\Utils\MultiValues::unserialize($this->_loadMeta($key));
        //dump($s);
...
//        dump($ret);
        ...
        foreach ($metaData as $key => $v) {
            $tagAssignment[$key] = \Bubo\Utils\MultiValues::unserialize($v['value']);
        }
        ...
        foreach($ret as $extTreeNodeId => $data) {
            if (isset($tagAssignment[$extTreeNodeId.'-tagAssignment']) &&
                ...
                //foreach ($langs as $langCode => $paramData) {
                    $out[$data['param_name']] = \Bubo\Utils\MultiValues::unserialize($data['param_value']);
                //}

                //$out[] = \Bubo\Utils\MultiValues::unserialize($value['param_value'])
...
//        die();
        ...
        foreach ($ret as $langCode => $values) {
            foreach ($values as $extParamId => $value) {
                ...
                $paramValues[$langCode][$extParamId] = \Bubo\Utils\MultiValues::unserialize($value['param_value']);
            }
        }
...
//            dump($properties, \Bubo\Utils\MultiValues::unserialize($data));
//            die();
        }
